...
export default function Home() {
  ...
  useEffect(() => {
    // Set the repos once the data is fetched
    setRepos(data);
  }, [data]);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6">
      {repos.map((repo, index) => (
        <div
          key={index}
          className="group relative border border-transparent rounded-xl w-72 shadow-lg transition-transform transform hover:scale-105 hover:shadow-xl"
        >
          <div className="absolute inset-0 bg-gradient-to-r from-purple-500 to-indigo-500 opacity-30 rounded-xl transition-opacity duration-500 group-hover:opacity-50"></div>
          <div className="relative z-10 bg-[#0D1224] border border-slate-700 rounded-xl p-4 shadow-md transition-transform duration-300 group-hover:translate-y-2 group-hover:shadow-lg">
            ...
            <div className="mt-4 text-center">
              <h3 className="font-bold text-xl text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-pink-500 transition-colors duration-300">
                {repo.name}
              </h3>
